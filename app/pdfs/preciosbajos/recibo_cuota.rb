...
class ReciboCuotaReport < Prawn::Document
  ...
  def initialize(pago)
    ...
    dibujar_pago(pago, 0.mm)
    move_cursor_to(self.bounds.top)

    dibujar_pago(pago, 110.mm)
    move_cursor_to(self.bounds.top)

    dibujar_pago(pago, 220.mm)
    move_cursor_to(self.bounds.top)
  end

  def dibujar_pago (pago, offset)
    move_down 9.mm
    ...
    bounding_box([10.mm, altura], :width => 195.mm) do
      text_box "#{Prawn::Text::NBSP * 50}"+ " #{numero_en_letras}",:size => 9, :leading => 14
    end
    ...
    bounding_box([10.mm, altura], :width => 195.mm) do
      ...
      if pago.pago_detalles.last.venta_cuota.estado==Settings.estadosCuotas.pagada
            ...
            text_box "#{Prawn::Text::NBSP * 25}"+ " Pago cuota N. #{numero_cuota_lista} según Factura Nro. #{pago.venta.nro_factura}",size:9
      else
        text_box "#{Prawn::Text::NBSP * 25}"+ " Pago parcial según Factura Nro. #{pago.venta.nro_factura}",size:9
      end
    ...
    altura = cursor
    if not (pago.numero_cheque.nil? or pago.numero_cheque.empty?)
      bounding_box([60.mm, altura], :width => self.bounds.right) do
        text "#{pago.numero_cheque}", size:9
      end
    else
      bounding_box([60.mm, altura], :width => self.bounds.right) do
        text "-", size:9
    ...
    if not (pago.banco_cheque.nil? or pago.banco_cheque.empty?)
      bounding_box([120.mm, altura], :width => self.bounds.right) do
        text "#{pago.banco_cheque}", size:9
      end
    else
      bounding_box([120.mm, altura], :width => self.bounds.right) do
        text "-", size:9
