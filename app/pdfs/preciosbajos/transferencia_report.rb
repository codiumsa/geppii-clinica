...
class TransferenciaReportPdf < Prawn::Document
  ...
  def dibujar_transferencia (transferencia, offset)
    ...
    sucursal = (Sucursal.where("deposito_id = ?", transferencia.destino_id)).first
    if sucursal

      if sucursal.vendedor
        nombre_vendedor = sucursal.vendedor.nombre
        apellido_vendedor =  sucursal.vendedor.apellido
      else
        nombre_vendedor = "Vendedor no definido para sucursal"
        apellido_vendedor = ""
      end

    else
      nombre_vendedor = "Sucursal no definida para depósito"
      apellido_vendedor = ""
    end
    ...
    bounding_box([54.mm, altura], :width => self.bounds.right) do
      text "#{nombre_vendedor} #{apellido_vendedor}",size:7
   	...
   	bounding_box([41.mm, altura], :width => self.bounds.right) do
    	text "#{transferencia.fecha_registro.strftime("%d/%m/%Y")}",size:7
    end
    ...
    pos_precio_venta = 85.mm
	   ...
	   transferencia.detalles.map do |detalle|
	      altura = cursor
        temp_codigo = "#{detalle.producto.codigo_barra}"
        if(temp_codigo.length > 13) #se acorta la cadena porque no entra en campo detalles
            loop do
              temp_codigo = temp_codigo.chop
              break if (temp_codigo.length == 12)
            end
            temp_codigo = temp_codigo + "~"
        end
        bounding_box([pos_codigo, altura], :width => 16.mm ,:height=> 3.mm) do
          text "#{temp_codigo}", size:5
        end
        temp_detalle_cantidad = "#{detalle.cantidad} #{detalle.producto.descripcion}"

        if(temp_detalle_cantidad.length > 31) #se acorta la cadena porque no entra en campo detalles
            loop do
              temp_detalle_cantidad = temp_detalle_cantidad.chop
              break if (temp_detalle_cantidad.length == 31)
            end
            temp_detalle_cantidad = temp_detalle_cantidad + "~"
        end
        bounding_box([pos_descripcion, altura], :width => 45.mm, :height=> 3.mm) do
          text "#{temp_detalle_cantidad}", size:5
        end
	precioDetalle = detalle.producto.precio_promedio * 1.5
	if (detalle.producto.tipo_producto.codigo == 'S')
	  precioDetalle = detalle.producto.precio	
	end
        bounding_box([pos_precio_venta, altura], :width => 19.mm, :height=> 3.mm) do
          text "#{to_Gs(precioDetalle * detalle.cantidad)}", size:5
        end

        move_down 2.mm
        total = total + precioDetalle * detalle.cantidad
      end
      ...
      bounding_box([23.mm, y_total - 4.mm], :width => self.bounds.right) do
        text "#{numero_a_palabras(total).upcase}", size:6
      end


      sucursal = Sucursal.where("deposito_id = ?", transferencia.origen_id).first
      ...
      bounding_box([47.mm, y_total - 20.mm], :width => self.bounds.right) do
         text "#{transferencia.descripcion}",size:7
       end
