...
class UsoInternoReportPdf < Prawn::Document
  ...
  def dibujar_venta (venta)
    ...
    altura = cursor
#    bounding_box([80, altura], :width => 220) do
 bounding_box([80, altura], :width => self.bounds.right) do

      if venta.nombre_cliente 
           text "#{venta.nombre_cliente}", size: 10
#        if(venta.nombre_cliente.length > 55) #se acorta la cadena porque no entra en campo detalles
#            temp_nombre = venta.nombre_cliente
#            loop do
#              temp_nombre = temp_nombre.chop
#              break if (temp_nombre.length == 55)
#            end
#            temp_nombre = temp_nombre + "..."
#        else
#          temp_nombre = venta.nombre_cliente
#        end
      else
...
#        if(venta.cliente.razon_social.length > 55) #se acorta la cadena porque no entra en campo detalles
#            temp_nombre = venta.cliente.razon_social 
#            loop do
#              temp_nombre = temp_nombre.chop
#              break if (temp_nombre.length == 55)
#            end
...
#        end
#      end
#      text "#{temp_nombre}", size:10
#    end
#    if(venta.usuario)
#      razon_social = venta.usuario.nombre + ' ' + venta.usuario.apellido
#      razon_social = razon_social.split.map { |i| i.capitalize }.join(' ')
#      bounding_box([300, altura], :width => 180) do
#        if(razon_social.length > 26) #se acorta la cadena porque no entra en campo detalles
#            temp_razon = razon_social
#            loop do
#              temp_razon = temp_razon.chop
#              break if (temp_razon.length == 26)
#            end
#            temp_razon = temp_razon + "..."
#        else
#          temp_razon = venta.cliente.razon_social
#        end
#        text "Cajero: #{temp_razon}", size: 10
      end
    end
#    bounding_box([500, altura], :width => self.bounds.right) do
#        text "Hora de venta: #{venta.created_at.strftime("%H:%M")}", size: 10
#    end
