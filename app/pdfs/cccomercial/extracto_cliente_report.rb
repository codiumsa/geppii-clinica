    ...
    @ventas.map do |venta|
      if venta.anulado == false
        ...
        venta.venta_cuotas.map do |c|
          ...
          cuota = CuotaPago.new
          if not c.pago_detalles.nil? and c.pago_detalles.count > 0
            i = 0
            c.pago_detalles.map do |p|
              ...
              cuota.monto =  i==0 ? c.monto : cuotas_pagos[i-1].saldo
              cuota.descuento = p.pago.descuento / p.pago.pago_detalles.count
