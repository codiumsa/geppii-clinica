...
class TransferenciaReportPdf < Prawn::Document
  ...
  def dibujar_transferencia (transferencia, offset)
    ...
    if nombre_vendedor.nil? or apellido_vendedor.nil?
      nombre_vendedor = ""
      apellido_vendedor = ""
    end
    ...
    transferencia.detalles.map do |detalle|
      ...
      bounding_box([pos_codigo, altura], :width => 15.mm ,:height=> 3.mm) do
        text "#{detalle.lote.producto.codigo_barra}", size:6
      end
      temp_detalle_cantidad = "#{detalle.cantidad} #{detalle.lote.producto.descripcion}"
      ...
      bounding_box([pos_precio_venta, altura], :width => 19.mm, :height=> 3.mm) do
        text "#{to_Gs(detalle.lote.producto.precio_promedio * 1.5 * detalle.cantidad)}", size:6
      end

      move_down 2.mm
      total = total + detalle.lote.producto.precio * detalle.cantidad
    end
    ...
    sucursal = Sucursal.where("deposito_id = ?", transferencia.origen_id);
    if !(sucursal.first.nil?)
      if(!sucursal.first.vendedor.nil?)
      ...
      nombre_vendedor = "Sin vendedor Asignado"
      apellido_vendedor = ""
    end
