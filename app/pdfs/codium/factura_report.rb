...
class FacturaReportPdf < Prawn::Document
  ...
  def dibujar_venta (venta, offset)
    offset_legal = 24.4.mm

    move_down 32.mm
    altura = cursor - (offset * 9.5.mm)

    bounding_box([131.mm, altura], :width => self.bounds.right) do
    ...
    move_down 4.mm
    altura = cursor
    bounding_box([37.mm, altura], :width => self.bounds.right) do
      text "#{venta.fecha_registro.strftime("%d/%m/%Y")}", size:7
    end
    if venta.credito.eql? false
      bounding_box([157.5.mm, altura], :width => self.bounds.right) do
        text "X", size:7
      end
    else
      bounding_box([193.5.mm, altura], :width => self.bounds.right) do
        text "X", size:7
      end
    end
    move_down 5.mm
    ...
    altura = cursor
    bounding_box([23.mm, altura], :width => self.bounds.right) do
    ...
    bounding_box([151.mm, altura], :width => self.bounds.right) do
    ...
    move_down 2.5.mm
    altura = cursor
    bounding_box([156.mm, altura], :width => self.bounds.right) do
      text "#{nombre_vendedor}", size:7
    end
    ...
    pos_exentas = 134.mm #la posicion de la columna de exentas en el eje horizontal
    pos_iva5 = 155.mm
    pos_iva10 = 179.mm

    pos_cantidad = 25.mm
    pos_descripcion = 40.mm
    pos_precio = 117.mm
    ...
    y_subtotales = 189.mm + offset_legal - (offset * 165.mm)#la posicion de la línea de subtotales respecto a la vertical (se cuenta desde abajo hacia arriba)
    y_down_totales = 6.mm
    ...
    bounding_box([32.mm, y_subtotales - y_down_totales], :width => self.bounds.right) do
      #text "#{to_Gs(Integer(total_a_pagar)).to_s + " (" + numero_en_letras + ")" }", size:7
      text " #{venta.moneda.simbolo}  #{to_Gs(Integer(total_a_pagar)).to_s}" + " (" + "#{venta.moneda.nombre.upcase} " + " #{numero_en_letras})", size:7
    end

    pos_total_iva5 = 41.mm
    pos_total_iva10 = 79.mm
    pos_total_iva = 125.mm
    
    total_iva = venta.iva5 + venta.iva10
    y_liq_iva = y_subtotales - (y_down_totales * 2) - 0.5.mm
