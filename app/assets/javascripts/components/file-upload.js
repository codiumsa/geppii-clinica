Bodega.FileUploadComponent = Ember.Component.extend({
  ...
  totalFileSize: function() {
      ...
      return Bodega.humanReadableFileSize(total);
  ...
  actions: {
    filesDropped: function(files) {
        ...
        if (multiple) {
          for(var i = 0; i < files.files.length; i++) {
            var fileUploadModel = Bodega.FileUploadModel.create({ fileToUpload: files.files[i] });
            model.pushObject(fileUploadModel);
          }
        } else {
          var fileUploadModel = Bodega.FileUploadModel.create({ fileToUpload: files.files[0] });
          model.pushObject(fileUploadModel);
    ...
    fileSelected: function(file) {
        ...
        var fileUploadModel = Bodega.FileUploadModel.create({ fileToUpload: file });
        model.pushObject(fileUploadModel);
        var multiple = this.get('multiple');
        if (!multiple) {
          this.set('showDropZone', false);
